...
def main():
    ...
    st.markdown(
        """
        <style>
        .css-1aumxhk { padding: 5px !important; }
        </style>
        """,
    ...
    with selected_patient_id:
        selected_patient_id.selectbox("Select a patient ID", patient_ids, index=patient_ids.index(selected_patient_id) if selected_patient_id in patient_ids else 0)

    if selected_patient_id:
        test_ids = df_specifics[df_specifics["patient_id"] == selected_patient_id]["test_id"].tolist()
    with selected_test_id:
        selected_test_id.selectbox("Select a test ID", test_ids, index=test_ids.index(selected_test_id) if selected_test_id in test_ids else 0)
